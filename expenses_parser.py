...
def parse_file(xlsx_path: Union[str, Path], out_root: Optional[Path] = None) -> Dict[str, Any]:
    """
    Алиас для вызова из pipeline и test_system.py.
    В отличие от parse_expenses_file() возвращает Dict с данными, а не путь к HTML.
    """
    xlsx_path = Path(xlsx_path)
    if not xlsx_path.exists():
        raise FileNotFoundError(f"Файл не найден: {xlsx_path}")

    out_root = out_root or Path(xlsx_path).resolve().parent.parent.parent  # .../GPT1C_Processor_temp
    parser = UnifiedExpensesParser(out_root=out_root)
    data = parser.parse_file(xlsx_path)
    slug = parser.generate_slug(xlsx_path, data)
    parser.save_json(data, slug)
    parser.save_html(data, slug)
    return data
