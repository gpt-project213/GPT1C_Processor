...
def analyze_response(
    ...
    system_prompt = (
        ...
        "  - paid_claim: клиент говорит, что уже оплатил или перевёл деньги. "
        "Примеры: 'скинул', 'скинули', 'закинул', 'закинули', 'перекинул', 'перевёл', 'отправил', "
        "'расчитался', 'рассчитался', 'оплатил', 'заплатил', 'расчет', 'уже оплачено', 'деньги отправлены', "
        "'QR/куар/киар прошёл', 'чек есть', 'деньги скоро упадут', 'оплата ещё не разнесена', "
        "'скинул на каспи', 'перевёл менеджеру', 'перечислил', 'перечислили'. "
        "НЕ требует чека в сообщении — достаточно утверждения об оплате.\n"
        "  - soft_positive: клиент ЯВНО сигнализирует о намерении платить, но без конкретной даты/суммы. "
        "Платёжные слова: 'оплачу', 'закрою', 'постараюсь', 'сегодня будет', 'переведу', 'оплатим', "
        "'скину', 'закину', 'перекину', 'отправлю', 'закроем'. "
        ...
        "requires_human: true если агрессия, юридические угрозы, "
        "или intent ∈ {cash_pickup, dispute, doc_request, complaint}\n"
        ...
        "suggested_reply при unclear: ОДИН короткий вопрос по теме долга — уточни дату оплаты "
        "или что имел в виду клиент. Не объясняй, не извиняйся, не рассуждай. "
        "Максимум 1–2 предложения. Если ответ клиента вообще не связан с долгом — спроси только "
        "про дату оплаты, игнорируй офтопик.\n"
    ...
    try:
        ...
        if start >= 0 and end > start:
            ...
            if data["intent"] in {"cash_pickup", "dispute", "doc_request", "complaint"}:
