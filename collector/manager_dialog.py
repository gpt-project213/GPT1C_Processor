...
async def handle_text_message(chat_id: int, text: str) -> bool:
    ...
    if state == STATE_AWAITING_CONFIRM:
        await _send_msg(
            chat_id,
            "⬆️ Чтобы ответить — нажмите кнопку под предыдущим запросом.\n\n"
            "Если кнопок не видно — пролистайте чат вверх "
            "или напишите /start, чтобы получить новый список запросов.",
        )
        return True
    ...
    if state == STATE_AWAITING_DATA_CONFIRM:
        await _send_msg(
            chat_id,
            "⬆️ Чтобы подтвердить или изменить — нажмите кнопку под предыдущим запросом.\n\n"
            "Если кнопок не видно — пролистайте чат вверх или напишите /start.",
        )
        return True
