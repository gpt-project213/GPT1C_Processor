# run_pipeline_all_mp.py · v1.4 · Asia/Almaty · 2025-09-19
# Оркестратор всех типов отчётов: DEBT / SALES / GROSS / INVENTORY
...
def _classify_by_content(path: Path) -> Optional[str]:
    """
    Порядок важен: DEBT → INVENTORY → SALES → GROSS.
...
def _process_one(src: Path) -> Tuple[str, List[Path]]:
    ...
    try:
        if routed_to == "DEBT":
            ...
            # на всякий случай — обрабатываем как DEBT
            outs = process_debt_file(work)
