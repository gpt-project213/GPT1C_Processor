...
async def _send_saida_intro(bot) -> None:
    ...
    instruction = (
        ...
        f"⏰ <b>Отвечай в течение {SAIDA_WARN_HOURS}ч.</b>\n"
        f"Через {SAIDA_WARN_HOURS}ч — придёт напоминание.\n"
        f"Через {SAIDA_BYPASS_HOURS}ч молчания — уведомления <b>уйдут клиентам автоматически</b>, "
    ...
    if data == "dstop_saida_help":
        await bot.send_message(
            ...
            text=(
                ...
                f"⏰ <b>Важно:</b> Отвечай на запросы в течение {SAIDA_WARN_HOURS}ч. "
                f"Через {SAIDA_BYPASS_HOURS}ч молчания уведомления уйдут клиентам автоматически, "
                "а менеджеры и директор узнают что ты не ответила."
            ),
        )
        return None
