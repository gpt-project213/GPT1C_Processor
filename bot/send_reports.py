...
async def cleanup_old_files(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if stats["logs"]["deleted"] > 0:
            ...
            message_parts.append("  📁 Логи (>2д): нет файлов для удаления")
        ...
        if stats["ai"]["deleted"] > 0:
            ...
            message_parts.append("  🤖 AI-отчёты (>7д): нет файлов для удаления")
        ...
        if stats["html"]["deleted"] > 0:
            ...
            message_parts.append("  📊 HTML-отчёты (>30д): нет файлов для удаления")
        ...
        if stats["json"]["deleted"] > 0:
            ...
            message_parts.append("  📋 JSON-отчёты (>7д): нет файлов для удаления")
        ...
        if stats["processed"]["deleted"] > 0:
            ...
            message_parts.append("  📦 Processed Excel (>14д): нет файлов для удаления")
...
async def check_and_send_silence_alerts(context=None):
    ...
    if all_managers_data and context:
        try:
            ...
            if ADMIN_CHAT_ID:
                ...
                logger.info("✅ Детальная сводка отправлена админу")
    ...
    logger.info("🔔 Проверка дней молчания завершена")
...
async def cb_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("archive|"):
        ...
        if action == "get" and len(parts) > 4:
            ...
            if not p or not p.exists():
                await q.answer("❌ Отчет не найден")
                types = _types_for_date_manager(manager, date)
                await _safe_edit_text(q.message, f"🗄️ **Архив → {manager} → {date}**\n\n❌ Отчет не найден",
                                    reply_markup=kb_archive_types(manager, date, types))
                return
    ...
    if data.startswith("ai_type|"):
        ...
        if report_type == "EXPENSES":
            await _safe_edit_text(q.message, "🤖 ИИ анализ затрат запущен...", reply_markup=None)
        ...
        if user_role == "manager":
            ...
            await _safe_edit_text(q.message, "🤖 ИИ анализ запущен...", reply_markup=None)
                ...
                if net_profit_failed:
                    try:
                        ...
                        if not expenses_files:
                            ...
                            alert_msg = (
                                "⚠️ *Чистая прибыль не рассчитана*\n\n"
                                "net_profit_report.py завершился с ошибкой.\n"
                                "Проверьте logs/ для деталей."
                            )
