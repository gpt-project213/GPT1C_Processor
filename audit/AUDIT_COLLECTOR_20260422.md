...
### F-006 — старые сообщения администратора могли оставаться активными после появления нового списка

- Файлы: `collector/approval_flow.py`, `collector/collections_engine.py`
- Симптом: при появлении нового актуального списка старые менеджерские сообщения уже закрывались, но старое админское сообщение могло остаться с рабочими кнопками
- Риск: нажатие по старому сообщению могло вернуть в работу уже неактуальный список и запутать финальный выбор
- Исправление:
  - при замене активного списка новым теперь закрываются и старые админские сообщения
  - `handle_admin_callback()` блокирует callback по статусам `superseded/expired/cancelled/sent/...`
  - ручной экран администратора теперь сразу убирает из списка уже обработанного клиента, чтобы не было повторного выбора по тому же элементу
- Доказательство:
  - тесты `APPROVAL T10i`, `APPROVAL T10j`, `APPROVAL T11d`
  - `python -m py_compile collector/approval_flow.py` — OK
  - `python -m py_compile collector/collections_engine.py` — OK
  - `tests/test_collector.py` — `269/269`

### F-007 — входящие голосовые WhatsApp падали на устаревшем параметре AssemblyAI

- Файл: `collector/whatsapp_poller.py`
- Симптом: голосовое доходило до скачивания, но транскрипция падала с `400` и текст не распознавался
- Корень: в запросе на `https://api.assemblyai.com/v2/transcript` передавался устаревший ключ `speech_model`; AssemblyAI требует новый контракт без этого поля
- Исправление:
  - удалён устаревший параметр `speech_model` из запроса транскрипции
- Доказательство:
  - боевой лог ранее содержал `AssemblyAI transcript ошибка 400 ... speech_model is deprecated`
