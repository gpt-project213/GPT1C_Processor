# AI_CONTEXT.md
<!-- AUTO-GENERATED by sync_project_to_github.ps1 - DO NOT EDIT MANUALLY -->
<!-- Updated: 2026-03-06 11:04:09 (Asia/Almaty) -->

## Project

**GPT1C_Processor / AI 1C PRO**

Automated pipeline for processing 1C Excel exports (debt, sales, gross profit, inventory, expenses),
generating HTML/JSON/PDF analytics reports, and delivering them via Telegram bot with AI analysis.

- Repository: https://github.com/gpt-project213/GPT1C_Processor
- RAW base: https://raw.githubusercontent.com/gpt-project213/GPT1C_Processor/master
- Branch: master
- Timezone: Asia/Almaty
- Language: Python 3.11+

---

## How AI Should Read This Project

Read files in this exact order to minimize wasted context:

1. AI_CONTEXT.md (this file) - project map
2. STATUS.md - current state and priorities
3. DEPENDENCY_MAP.md - full import graph and data flow (9 layers)
4. repo_map.json - machine-readable file index
5. raw_links.txt - direct RAW URLs to all files
6. Code: start with core layer, then parsers, then reports, then bot

---

## Architecture (9 Layers)

Layer 0: utils_common.py, send_tg.py, cleanup_cache.py, tools/
  Role: Utilities with no project imports

Layer 1: config.py
  Role: Paths, TZ, logging, managers config

Layer 2: utils_excel.py, utils.py
  Role: Excel cleaning, Jinja env, formatters

Layer 3: *_parser.py
  Role: xlsx to DataFrame/dict

Layer 4: debt/sales/gross/inventory/expenses _report.py
  Role: Excel to HTML and JSON

Layer 5: dso/rfm/concentration/turnover/net_profit/profitability
  Role: JSON to analytics HTML (safe to modify, no project imports)

Layer 6: ai_analyzer.py
  Role: HTML to AI txt/html via DeepSeek or OpenAI

Layer 7: imap_fetcher.py, run_pipeline*.py
  Role: Orchestrators

Layer 8: bot/send_reports.py + bot/*.py
  Role: Telegram UI (v9.4.19)

---

## Entry Points

bot/send_reports.py       - Main bot (v9.4.19), scheduler, menu, delivery
run_pipeline_all_mp.py    - Multi-process pipeline (all report types)
run_pipeline.py           - Simplified pipeline (debt only)
imap_fetcher.py           - IMAP email to reports/queue/
run_new_reports_now.py    - Manual trigger

---

## Parsers (Layer 3)

- expenses_parser.py
- gross_parser.py
- inventory_cost_parser.py
- sales_parser.py

---

## Core Report Generators (Layer 4)

debt_auto_report.py     : xlsx -> reports/html/debt_*.html + reports/json/debt_*.json
sales_report.py         : xlsx -> reports/html/sales_*.html
gross_report.py         : xlsx -> reports/html/gross_*.html
gross_report_pct.py     : xlsx -> reports/html/gross_pct_*.html
inventory.py            : xlsx -> reports/html/inventory_*.html + reports/json/
expenses_report.py      : xlsx -> reports/html/expenses_*.html + reports/json/

---

## Analytics (Layer 5) - reads JSON, writes reports/analytics/

dso_aging_report.py              : reads debt + sales JSON
rfm_clients_report.py            : reads sales JSON
revenue_concentration_report.py  : reads sales JSON
inventory_turnover_report.py     : reads inventory + sales JSON
net_profit_report.py             : reads gross + expenses JSON
sales_profitability_report.py    : reads sales + gross JSON

---

## Bot Modules (Layer 8)

- bot/gross_summary.py
- bot/inject_local.py
- bot/inventory_summary.py
- bot/opportunity_loss.py
- bot/sales_summary.py
- bot/send_reports.py
- bot/silence_alerts.py
- bot/user_tracker.py

---

## HTML Templates

- templates/base.html
- templates/debt_auto.html
- templates/gross.html
- templates/gross_percent.html
- templates/inventory_simple.html
- templates/sales_by_product.html
- templates/sales_report_grouped.html

---

## AI Prompts

- ╨Я╨а╨Ю╨Ь╨в_╨Ф╨Ы╨п_╨Т╨Р╨Ы╨Ю╨Т╨Ю╨Щ.txt
- ╨Я╨а╨Ю╨Ь╨в_╨Ф╨Ы╨п_╨Ф╨Х╨С╨Ш╨в╨Ю╨а╨Ъ╨Ш.txt
- ╨Я╨а╨Ю╨Ь╨в_╨Ф╨Ы╨п_╨Ч╨Р╨в╨а╨Р╨в.txt
- ╨Я╨а╨Ю╨Ь╨в_╨Ф╨Ы╨п_╨Ю╨б╨в╨Р╨в╨Ъ╨Ю╨Т.txt
- ╨Я╨а╨Ю╨Ь╨в_╨Ф╨Ы╨п_╨Я╨а╨Ю╨Ф╨Р╨Ц.txt
- DEPENDENCY_MAP.md.txt
- raw_links.txt
- requirements.txt

---

## Data Flow (summary)

Email (IMAP)
  imap_fetcher.py -> reports/queue/
    run_pipeline_all_mp.py routes by filename:
      DEBT    -> debt_auto_report.py  -> html + json/debt_*.json
      SALES   -> sales_report.py      -> html + json/sales_*.json
      GROSS   -> gross_report.py      -> html + json/gross_*.json
      INVENT  -> inventory.py         -> html + json/inventory_*.json
      EXPENSE -> expenses_report.py   -> html + json/expenses_*.json

reports/json/ -> Analytics layer (dso, rfm, concentration, turnover, net_profit)
             -> reports/analytics/

reports/html/ -> ai_analyzer.py -> reports/ai/ (txt + html)

bot/send_reports.py (APScheduler):
  every 10 min : run_pipeline_all_mp.py
  09:00        : inventory summary -> admin
  14:00        : silence_alerts + opportunity_loss -> managers
  20:00        : gross report
  21:00        : sales + silence_alerts
  22:00        : daily_analytics (all 6 analytics)
  23:00        : daily_summary -> admin
  Mon 10:00    : weekly_analytics

---

## Config Files

.env                       : TG_BOT_TOKEN, DEEPSEEK_API_KEY, TZ, paths
config/managers.json       : Manager names and chat_id
config/roles.json          : Admin, subadmin_scopes
config/imap.json           : IMAP server credentials
config/pattern_config.yaml : Regex for report type detection

---

## Known Issues

D1: expenses_parser.py and expenses_report.py are duplicates (same class UnifiedExpensesParser)
D4: sales_parser.py reads managers.json in wrong format (expects name/aliases, actual is name:chat_id)
Bot version: v9.4.19 (latest stable)
Active development: opportunity_loss analytics, token optimization Phase 3

---

## Roles

admin    : Vadim (7422963573)     - sees everything
subadmin : Alena (188939016)      - sees Magira + Oksana reports
manager  : Oksana, Magira, Ergali - sees own reports only

