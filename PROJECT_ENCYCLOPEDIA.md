...
- **partial payment автоматизация** — 100% оплата → автоснятие долга работает только для «чистых» клиентов. Проблемные клиенты (только предоплата / отказать) требуют решения директора. В CRM нет поля `release_policy` (`auto` / `prepayment_only` / `blacklist`) — без него автоматизировать нельзя. Реализовывать когда директор готов расставить флаги.
