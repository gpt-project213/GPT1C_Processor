...
- **Инструменты качества установлены** (2026-05-21): `ruff` — 427 косметических правок (пробелы, f-строки); `bandit` — 4 предупреждения (hashlib, не критично); `vulture` — мёртвый код найден; `pip-audit` — 20 CVE закрыто обновлением пакетов
- **Пакеты обновлены до безопасных версий** (2026-05-21): `python-dotenv→1.2.2`, `setuptools→82.0.1`, `jinja2→3.1.6`, `urllib3→2.7.0`, `idna→3.15`, `aiohttp→3.13.5`, `pyarrow→24.0.0`; `pip-audit: No known vulnerabilities found`
- **Ротация логов** — уже работала (`TimedRotatingFileHandler`, midnight, 14 дней); **авторестарт** — уже работал (`start_bot_watchdog.bat`, 5 сек/30 сек)
...
- ~~**`txt_to_html` дублирование**~~ — закрыто как намеренное: `bot/send_reports.py` (2 аргумента, Telegram-доставка) и `tools/txt_to_html.py` (1 аргумент, CLI, JSON-aware). Разные контракты, унификация не даёт пользы. Документировано `ARCH-1` в `send_reports.py`.
- **`debt_age_history.json` bootstrap** — бот накопит сам за 1-2 месяца.
- **partial payment автоматизация** — статичный флаг «проблемный/нормальный» не работает: клиент может быть надёжным год, потом начать задерживать. Автоматически снимать долг по 100% оплате безопасно только если поведение клиента предсказуемо — а это видно только из истории. Реальная автоматизация = бот готовит директору справку (история просрочек, % вовремя, прецеденты частичной оплаты), решение остаётся за человеком.
