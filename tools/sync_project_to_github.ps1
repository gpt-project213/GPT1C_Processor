...
function Update-Status {
    ...
    if (-not $hasAiContext)        { $gapLines += "- AI_CONTEXT.md (auto-generated)" }
...
function Update-AiContext {
    ...
    $lines += "  Role: Telegram UI (v9.4.19)"
    $lines += ""
    $lines += "---"
    $lines += ""
    $lines += "## Entry Points"
    $lines += ""
    $lines += "bot/send_reports.py       - Main bot (v9.4.19), scheduler, menu, delivery"
    ...
    $lines += "      DEBT    -> debt_auto_report.py  -> html + json/debt_*.json"
    ...
    $lines += "## Known Issues"
    $lines += ""
    $lines += "D1: expenses_parser.py and expenses_report.py are duplicates (same class UnifiedExpensesParser)"
    $lines += "D4: sales_parser.py reads managers.json in wrong format (expects name/aliases, actual is name:chat_id)"
    $lines += "Bot version: v9.4.19 (latest stable)"
    $lines += "Active development: opportunity_loss analytics, token optimization Phase 3"
    $lines += ""
    $lines += "---"
    $lines += ""
    $lines += "## Roles"
