...
| Коммит | Что |
|---|---|
| `893877c` | fix: lazy-expiry `pending_saida` в `get_hold_for_client` |
| `a06fe22` | feat: `expire_stale_holds_on_startup()` + уведомление Минай при новом hold |
| `527d6d5` | fix: убрать prefix менеджера в сообщении Минай |
| `8ff2f1c` | feat: `strip_manager_prefix()` во всех внешних сообщениях |
| `7fc2d29` | feat: `bot/minai_reminders.py` — полный WhatsApp-контур |
| `efc7505` | feat: подсказка «написать или сказать голосовым» |
| `d4a999b` | fix: авто-закрытие зависших состояний (30 мин таймаут) |
| `783dd05` | fix: критичный день — финальный алерт + уведомление Вадима в 22:00 |
| `eadf86d` | feat: рабочее vs личное напоминание |
| `ed96d30` | feat: приветственное сообщение при первом запуске |
| `6cfebab` | feat: фидбек → Вадиму + «создан специально для тебя» |
| `319b4ca` | fix: catch-all для нераспознанных сообщений |

### Ключевые бизнес-правила

- `pending_saida` hold больше не блокирует collector вечно: TTL 12h, lazy-expiry прямо при чтении
- Минай (`MINAI_WA_PHONE=77023177888`) получает WA при каждом новом payment hold — чтобы предоставила выписку Саиде вовремя
- Префикс менеджера ("О", "Е", "М", "А") убран из ВСЕХ внешних сообщений (WA клиентам, Саиде, Минай)

### Тесты (HEAD `319b4ca`)

| Suite | Результат |
|---|---|
| `test_collector.py` | **657/657** |
| `test_collector_regression_hermetic.py` | **43/43** |
| `test_crm_regression.py` | 44/44 |
| `test_project.py` | 110/110 |

### Что остаётся открытым

- Перезапуск бота для загрузки нового кода (catch-all, рабочее/личное, фидбек)
- Приветствие Минай отправлено в 08:43 19.05 — не повторится при следующем рестарте
- `debt_age_history.json` bootstrap из архива — опционально
...
#### CRM state lock — hard-fail policy (`0c8561e`)

Заменён best-effort yield на `CrmStateLockError` + явный контракт для callers:
- `_crm_state_lock()` → raise `CrmStateLockError` при недоступности lock (не yield)
- Все 4 save-функции → `bool`; при `CrmStateLockError` → `crm_state_lock_timeout` лог + `return False`
- Load-функции: startup tolerant, только error-лог
- Callback callers (claim, dup pick/distinct/custom, crm_name/phone): if not save → rollback in-memory + `"⚠️ Временная ошибка сохранения"`
- Rollback/scheduler callers: log error, proceed
- Тесты (+2): `test_crm_state_lock_raises_on_contention`, `test_crm_save_pending_returns_false_on_lock_fail`

#### Stale silent dialogs унблокированы (`d4ae395`)

Причина: Шахин, Тян, Еркебулан залипли в `active, exchange_count=0` — никогда не попадали в следующий batch.

- `collector/client_dialog.py` v1.1.10: `dialog_blocks_new_outreach(dialog)` — единый предикат
  - `active` + 0 ответов + 24h+ → `False, "stale_silent_active"` (разблокировать)
  - `awaiting_payment_proof` + 72h+ → `False, "stale_payment_proof"`
  - иначе → `True, state`
- `start_client_dialog()` при supersede stale-диалога пробрасывает `phone_silent_cycles`
- `collections_engine.py` (preview + send-approved): переведён на `dialog_blocks_new_outreach()`; legacy TTL-ветка удалена из обоих путей
- `collector/approval_penalty.py` v1.1.2: `build_reset_state()` + `wa_reset_floor`/`crm_reset_floor` floor-метки: батчи/CRM-записи старше floor при reset не получают штраф при backfill

#### Admin penalty reset button (`f7f1d13`)

- `collector/approval_penalty.py`: `reset_penalty_state()` — backup + `build_reset_state()` + write
- `bot/send_reports.py`: кнопка `♻️ Сброс штрафов` в экране 🤖 Коллектор; confirm-step (admin only); после подтверждения показывает месяц, WA floor, CRM floor, имя backup
...
- live smoke-test после рестарта бота
- stale dup-review callback: full cleanup-строгость как у claim (minor)
- `COLLECTOR_SILENT_ACTIVE_RESEND_HOURS` (default 24h) — проверить порог в бою
- `reset_penalty_state()` подключена к UI; при необходимости расширить до CLI-утилиты
